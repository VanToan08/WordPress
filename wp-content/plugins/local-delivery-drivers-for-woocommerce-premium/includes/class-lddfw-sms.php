...
class LDDFW_SMS {
	...
	public function lddfw_send_sms_twilio__premium_only( $sms_text, $from_number, $to_number, $sid, $auth_token ) {
		...
		if ( ( ! empty( $xml->SMSMessage[0]->Status[0] ) ) && 'queued' === strval( $xml->SMSMessage[0]->Status[0] ) ) {
			/* translators: %s: phone number */
			return array( 1, sprintf( __( 'SMS has been sent successfully to %s', 'lddfw' ), $to_number ) );
		} else {
			/* translators: %s: phone number */
			return array( 0, sprintf( __( 'Failed to send SMS to %s ' . $xml->RestException[0]->Message[0], 'lddfw' ), $to_number ) );
		}
	}
}
