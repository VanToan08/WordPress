...
class WooHook {
	...
	public function __construct() {
		add_filter( 'woocommerce_rest_prepare_product_review', array(
			$this,
			'woocommerce_rest_prepare_product_review'
		), 10, 3 );

		add_action( 'woocommerce_rest_insert_product_review', array(
			$this,
		 ...
		 */
		add_filter( 'posts_clauses', [ $this, 'filter_nearby_product' ], 501, 2 );
	}
	...
	public function woocommerce_rest_prepare_product_object( $response, $post, $request ) {
		...
		if ( class_exists( 'WC_Product_Addons_Helper' ) ) {
			...
			if ( count( $product_addons ) > 0 && isset( $data['meta_data'] ) ) {
				$meta_data = is_array( $data['meta_data'] ) ? $data['meta_data'] : [];
