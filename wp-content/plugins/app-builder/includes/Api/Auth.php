...
class Auth extends Base {
	...
	public function register_routes() {
		...
		register_rest_route( $this->namespace, 'register-phone-number', array(
			'methods'             => WP_REST_Server::CREATABLE,
			'callback'            => array( $this, 'register_phone_number' ),
			'permission_callback' => '__return_true',
		) );

		register_rest_route( $this->namespace, 'current', array(
		...
		register_rest_route( $this->namespace, 'login-token', array(
			'methods'             => WP_REST_Server::READABLE,
		...
		register_rest_route( $this->namespace, 'send-otp-delete', array(
			'methods'             => WP_REST_Server::CREATABLE,
	 ...
	 */
	public function pre_data_response( $user, $request ) {
	...
	public function register_phone_number( $request ) {
		...
		try {
			 ...
			 */
			if ( count( $users ) > 0 ) {
				return $this->pre_data_response( $users[0], $request );
			}
	...
	private function verify_role( $role ): string {
		if ( ! $role || ! in_array( $role, [ 'subscriber', 'customer', 'wcfm_vendor' ] ) ) {
