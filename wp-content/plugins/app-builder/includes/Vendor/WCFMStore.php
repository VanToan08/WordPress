...
class WCFMStore extends BaseStore {
	...
	public function get_settings( $request ) {
		...
		if ( isset( $vendor_settings_data['gravatar'] ) ) {
			$image                                = wp_get_attachment_image_src( $vendor_settings_data['gravatar'], 'full' );
			$vendor_settings_data['gravatar_src'] = current( $image );
		}

		if ( isset( $vendor_settings_data['banner'] ) ) {
			$image                              = wp_get_attachment_image_src( $vendor_settings_data['banner'], 'full' );
			$vendor_settings_data['banner_src'] = current( $image );
		}

		if ( isset( $vendor_settings_data['mobile_banner'] ) ) {
			$image                                     = wp_get_attachment_image_src( $vendor_settings_data['mobile_banner'], 'full' );
			$vendor_settings_data['mobile_banner_src'] = current( $image );;
		}
		...
		if ( isset( $vendor_settings_data['store_seo'] ) ) {
			...
			if ( isset( $store_seo['wcfmmp-seo-twitter-image'] ) && $store_seo['wcfmmp-seo-twitter-image'] ) {
				$image                                     = wp_get_attachment_image_src( $store_seo['wcfmmp-seo-twitter-image'], 'full' );
				$store_seo['wcfmmp-seo-twitter-image-src'] = current( $image );
			}

			if ( isset( $store_seo['wcfmmp-seo-og-image'] ) && $store_seo['wcfmmp-seo-og-image'] ) {
				$image                                = wp_get_attachment_image_src( $store_seo['wcfmmp-seo-og-image'], 'full' );
				$store_seo['wcfmmp-seo-og-image-src'] = current( $image );
			}

			$vendor_settings_data['store_seo'] = $store_seo;
		}

		return rest_ensure_response( array_merge( $wcfm_policy_vendor_options, $vendor_settings_data ) );
	}
	...
	public function get_profile( $request ) {
		...
		foreach ( $this->wcfm_profile_personal_fields as $key => $field ) {
			...
			if ( $key == "fd_user_avatar" ) {
				$image                  = wp_get_attachment_image_src( $personal[ $field ], 'full' );
				$personal['avatar_src'] = current( $image );
			}
